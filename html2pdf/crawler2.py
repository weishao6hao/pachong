    ...
    def run(self):
        ...
        for index, url in enumerate(self.parse_menu(self.crawl(self.start_url))):
            ...
            with open(f_name, 'wb') as f:
                f.write(html)
            htmls.append(f_name)
        path_wk = r'D:\Program Files\wkhtmltox\wkhtmltopdf\bin\wkhtmltopdf.exe'  # 安装位置
...
class LiaoxuefengPythonCrawler(Crawler):
    ...
    def parse_menu(self, response):
        ...
        menu_tag = soup.find('ol', attrs={'class': 'toc'})
        for li in menu_tag.find_all("li"):
            if li.a:
                if 'chapter' in li.a.get("href"):
                    url = li.a.get("href")
                    url='/'+url
    ...
    def parse_body(self, response):
        ...
        try:
            soup = BeautifulSoup(response.content, 'html.parser')
            body = soup.find_all(class_="yui-b")[0]
...
if __name__ == '__main__':
    start_url = "http://djangobook.py3k.cn/2.0/"
    crawler = LiaoxuefengPythonCrawler("The Django Book", start_url)
    crawler.run()
