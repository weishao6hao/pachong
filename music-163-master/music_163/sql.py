...
def get_all_artist():
    with connection.cursor() as cursor:
        sql = "SELECT `ARTIST_ID` FROM `artists` ORDER BY ARTIST_ID limit 0,100 "
        cursor.execute(sql, ())
...
def get_before_music():
    with connection.cursor() as cursor:
        sql = "SELECT `MUSIC_ID` FROM `musics` ORDER BY MUSIC_ID LIMIT 0, 50000"
        cursor.execute(sql, ())
        return cursor.fetchall()
...
def get_after_music():
    with connection.cursor() as cursor:
        sql = "SELECT `MUSIC_ID` FROM `musics` ORDER BY MUSIC_ID LIMIT 500000, 1197429"
        cursor.execute(sql, ())
        return cursor.fetchall()
